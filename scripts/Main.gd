...
func _ready() -> void:
    ...
    _boss_hp_bar.position = Vector2((viewport_rect.size.x - 240) / 2, 60)
...
func _process(delta: float) -> void:
    ...
    if _boss_hp_bar.visible:
        ...
        if boss != null and "current_hp" in boss:
...
func _on_item_purchased(unit_type: int, cost: int) -> void:
    # Validate purchase on Main side as well
    if gold < cost:
        print("Error: Not enough gold for purchase!")
        return
