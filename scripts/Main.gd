...
func _on_item_purchased(unit_type: int, cost: int) -> void:
    print("[Main] Item purchased: ", unit_type, " Cost: ", cost, " Current Gold: ", gold)
    # Ensure we have enough gold (ShopUI checks too, but safe is safe)
    if gold >= cost:
        gold -= cost
        _update_ui()
        ...
        print("[Main] Error: Not enough gold! Gold: ", gold, " Cost: ", cost)
