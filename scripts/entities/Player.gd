...
func _add_body_internal(unit_type: int, level: int, insert_index: int = -1) -> void:
    ...
    if matching_indices.size() == 2:
        # Found 2 existing + current 1 = 3. Merge!
        var target_idx = matching_indices.min()
        if insert_index != -1:
            target_idx = min(target_idx, insert_index)
    ...
    # Snap position to target to avoid huge jumps
