...
func _add_body_internal(unit_type: int, level: int, insert_index: int = -1) -> void:
    ...
    if matching_indices.size() == 2:
        # Found 2 existing units, plus the new one makes 3. Merge!
        var target_index = min(matching_indices[0], matching_indices[1])
        if insert_index != -1:
            target_index = min(target_index, insert_index)
        ...
        print("Merged 3x type %d Level %d -> 1x Level %d at index %d" % [unit_type, level, level+1, target_index])
    ...
    if target:
        var offset = body.get("follow_offset")
